...
import { View, FlatList, TextInput, Pressable, KeyboardAvoidingView, Platform, Alert, ScrollView } from 'react-native';
...
export default function ChatRoomScreen() {
  ...
  const { currentUser, users, chats, sendMessage, editMessage, deleteMessage, updateMessageStatus } = useAppContext();
  ...
  const [images, setImages] = useState<Array<{ uri: string, previewUri: string; }>>([]);
  ...
  const handleSendMessage = () => {
    if ((!messageText.trim() && images.length === 0) || !currentUser || !chat) return;
  ...
  const handleDeleteMessage = (messageId: string) => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Delete', style: 'destructive', onPress: () => deleteMessage(chat?.id || '', messageId) },
      ]
    );
  };
  ...
  useEffect(() => {
    if (flatListRef.current && chat?.messages.length && chat?.messages.length > 0) {
  ...
  useEffect(() => {
    if (chat && currentUser) {
      const unreadMessages = chat.messages.filter(
        msg => msg.senderId !== currentUser.id && msg.status !== MessageStatus.READ
      );

      unreadMessages.forEach(msg => {
        updateMessageStatus(chat.id, msg.id, MessageStatus.READ);
      });
    }
    ...
    >
      ...
      <Stack.Screen 
        options={{
          ...
          headerRight: () => !inputSearchdVisible ? (
            <Pressable style={styles.sendButton}
              onPress={() => setInputSearchdVisible(true)}
            >
              <IconSymbol name="questionmark" size={24} color={ThemeColors.blue} />
            </Pressable>
          ) : (
            <Pressable style={styles.sendButton}
                onPress={() => { setInputSearchdVisible(false); setSearchText(''); }}
            >
              <IconSymbol name="timer.square" size={24} color={ThemeColors.blue} />
            </Pressable>
          )
        }} 
      />
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
            ...
            onLongPress={() => {
              if (isCurrentUser) {
                Alert.alert(
                  ...
                  [
                    ...(item.imageUri ? [] : [{ text: 'Edit', onPress: () => handleEditMessage(item.id, item.text) }]),
          ...
          >
            {item.imageUri ? (
              <Image
                style={[styles.messageImage, { alignSelf: isCurrentUser ? 'flex-end' : 'flex-start' }]}
                uri={item.imageUri}
      ...
      <ThemedView style={styles.inputContainer}>
        ...
        >
          {images.map((img, index) => (
            <View key={index} style={[styles.imageContainer, { marginLeft: index === 0 ? 10 : 0, }]}>
              <Image
                style={styles.image}
                uri={img.uri}
        ...
        <View style={styles.inputRow}>
          ...
          <Pressable
            style={[
              styles.sendButton,
              !messageText.trim() && images.length === 0 && styles.disabledButton
            ]}
            onPress={handleSendMessage}
            // disabled={!messageText.trim() && selectedImages.length === 0}
          >
            <IconSymbol
              name={editingMessageId ? "checklist.checked" : "arrow.up.circle.fill"}
              size={32}
